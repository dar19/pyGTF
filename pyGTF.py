...
class Sequence(object):
    ...
    def reverse_complement(self):
        ...
        tmp = Sequence(
            self.name, "".join([paired_rc[i] for i in self.seq[::-1]]), self.descr
        )
        return tmp
...
class Transcript(object):
    ...
    def __init__(self, Tid, chro, start, end, strand, exon, cds, infor, suffix=""):
        ...
        assert any(
            [exon, cds]
        ), 'missing "exon" and "CDS" feature of transcript "{}"'.format(Tid)
    ...
    def __valid_data(self):
        ...
        if self.__exon:
            ...
            info = (
                "Error: Over range of the transcriptional region.\n",
    ...
    def to_gtf(self, fp):
        ...
        identifier = 'transcript_id "{}"; gene_id "{}"; '.format(
        ...
        if self.__cds:
            for i, each in enumerate(self.__cds):
                start, end = each
                Feature.append(
                    [self.chro, ".", "CDS", start + 1, end, ".", self.strand, ".", attr]
                )
        UTR = self.__UTR_identify()
        for utr in UTR:
            labels, start, end = utr
            Feature.append(
                [self.chro, ".", labels, start + 1, end, ".", self.strand, ".", attr]
            )
    ...
    def extract_genomic_seq(self, seq_dict, fp):
        ...
        wanted = Sequence(
            "{}_genomic".format(self.name), seq, "gene_id:{}".format(self.gene_id)
        )
        if self.strand == "-":
    ...
    def extract_intron_seq(self, seq_dict, fp):
        ...
        if intron_num >= 1:
            for index in range(intron_num):
                ...
                wanted = Sequence(
                    "{}_intron{}".format(self.name, order),
                    seq_dict[self.chro][start:end],
                    "gene_id:{}".format(self.gene_id),
                )
                # seqlist.append(wanted)
    ...
    def extract_UTR_seq(self, seq_dict, fp):
        ...
        if UTR:
            ...
            for index, utr in enumerate(UTR):
                ...
                #                seq_dict[self.chro][start:end]))
                wanted = Sequence(
                    "{}_{}_{}".format(self.name, labels, index),
                    seq_dict[self.chro][start:end],
                    "gene_id:{}".format(self.gene_id),
                )
                wanted = wanted.reverse_complement() if self.strand == "-" else wanted
    ...
    def __iter__(self):
        ...
        for line in Files.__iter__(self):
            ...
            try:
                ...
                line_id = (
                    attr["ID"] if feature in ["transcript", "mRNA"] else attr["Parent"]
                )
        ...
        yield Transcript(
            t_id, t_chro, t_start, t_end, t_strand, t_exon, t_cds, t_info, self.suffix
        )
...
class GTF_Reader_Saft(Files):
    ...
    def __iter__(self):
        for t_id in self.annot:
            ...
            yield Transcript(
                t_id, chro, start, end, strand, exon, cds, t_info, self.suffix
            )
    ...
    def __iter__(self):
        ...
        for line in Files.__iter__(self):
            ...
            if feature == "gene":
                ...
                print(
                    "Feature:({}) is ignore, please check line:\n    {}".format(
                        feature, line
                    )
                )
        yield Transcript(
